...
@section('title', trans('dune-rp::messages.characters.edit'))

@push('styles')
    <link href="{{ plugin_asset('dune-rp', 'css/dune-rp.css') }}" rel="stylesheet">
    @if($character->house)
        <style>
            .character-edit-theme {
...
@section('content')
<div class="dune-container character-edit-theme">
    {{-- Header Section --}}
    <div class="page-header dune-panel" style="text-align: center; padding: 40px 20px; background: linear-gradient(135deg, rgba(139,0,0,0.3), {{ $character->house ? $character->house->color.'40' : 'rgba(30,74,140,0.3)' }});">
        ...
        <p style="font-size: 1.1rem; color: var(--dune-sand); max-width: 700px; margin: 0 auto;">
            Peaufinez les détails de <strong style="color: var(--character-house-color, var(--dune-spice-glow));">{{ $character->name }}</strong> 
            et continuez à écrire votre légende.
        </p>
        
        @if(!$character->is_approved)
            <div style="margin-top: 20px; padding: 15px; background: rgba(255,152,0,0.2); border: 2px solid #ff9800; border-radius: 10px; max-width: 500px; margin-left: auto; margin-right: auto;">
                <i class="bi bi-hourglass-split" style="color: #ff9800;"></i>
                <strong style="color: #ff9800;">En attente d'approbation</strong> - Les modifications importantes nécessiteront une nouvelle validation.
            </div>
        @endif
    </div>
    ...
    <div class="character-form-container" style="max-width: 900px; margin: 30px auto;">
        <form method="POST" action="{{ route('dune-rp.characters.update') }}" enctype="multipart/form-data" class="dune-form">
            @csrf
            @method('PUT')
            ...
            <div class="form-sections" style="display: grid; grid-template-columns: 1fr; gap: 30px;">
                ...
                <div class="character-preview dune-panel" style="padding: 25px; background: linear-gradient(135deg, rgba(15,15,35,0.9), {{ $character->house ? $character->house->color.'20' : 'rgba(214,166,95,0.1)' }});">
                    <h2 style="margin: 0 0 20px 0; color: var(--character-house-color, var(--dune-spice-glow)); text-align: center; font-size: 1.4rem;">
                        <i class="bi bi-person-badge"></i> Aperçu Actuel
                    </h2>
                    
                    <div style="display: flex; align-items: center; gap: 25px; max-width: 600px; margin: 0 auto;">
                        <div class="current-avatar" style="width: 80px; height: 80px; border: 3px solid var(--character-house-color, var(--dune-sand)); border-radius: 50%; overflow: hidden;">
                        ...
                        <div class="current-info">
                            <h3 style="margin: 0 0 5px 0; color: var(--character-house-color, var(--dune-spice-glow)); font-size: 1.3rem;">
                                {{ $character->name }}
                            </h3>
                            @if($character->title)
                            ...
                            @endif
                            @if($character->house)
                                <p style="margin: 0; color: var(--character-house-color, var(--dune-sand)); font-weight: bold;">
                                    <i class="bi bi-shield"></i> {{ $character->house->name }}
                                </p>
                            @endif
                        </div>
                        
                        <div class="current-status" style="margin-left: auto;">
                            <span class="status-badge status-{{ $character->status }}" style="padding: 6px 12px; border-radius: 15px; font-size: 0.8rem; font-weight: bold; color: white;">
                ...
                <div class="form-section dune-panel" style="padding: 30px;">
                    <h2 class="section-title dune-heading" style="margin-bottom: 25px; color: var(--character-house-color, var(--dune-spice-glow)); font-size: 1.6rem;">
                ...
                <div class="form-section dune-panel" style="padding: 30px;">
                    ...
                    @if($character->house_id != old('house_id', $character->house_id))
                        <div class="house-change-warning" style="margin-bottom: 20px; padding: 15px; background: rgba(255,152,0,0.2); border: 2px solid #ff9800; border-radius: 8px;">
                            <i class="bi bi-exclamation-triangle" style="color: #ff9800;"></i>
                            <strong style="color: #ff9800;">Attention:</strong> Changer de maison nécessitera une nouvelle approbation par un administrateur.
                        </div>
                    @endif
                            ...
                            <div class="houses-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 15px;">
                                ...
                                <div class="house-option">
                                    <input type="radio" id="house_none" name="house_id" value="" 
                                           {{ old('house_id', $character->house_id) == '' ? 'checked' : '' }} style="display: none;">
                                ...
                                @foreach($houses as $house)
                                    <div class="house-option">
                                        <input type="radio" id="house_{{ $house->id }}" name="house_id" value="{{ $house->id }}" 
                                               {{ old('house_id', $character->house_id) == $house->id ? 'checked' : '' }} style="display: none;">
                                        <label for="house_{{ $house->id }}" class="house-card" style="display: block; padding: 20px; border: 2px solid {{ $house->color ?? 'var(--dune-sand)' }}; border-radius: 10px; cursor: pointer; text-align: center; transition: all 0.3s;">
                                            ...
                                            @if($character->house_id == $house->id)
                                                <div style="margin-top: 10px; padding: 4px 8px; background: {{ $house->color ?? 'var(--dune-spice)' }}; color: white; border-radius: 10px; font-size: 0.7rem; font-weight: bold; display: inline-block;">
                                                    ACTUEL
                                                </div>
                                            @endif
                                        </label>
                ...
                <div class="form-section dune-panel" style="padding: 30px;">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="biography" name="biography" class="dune-textarea" rows="8" maxlength="5000" placeholder="Racontez l'histoire de votre personnage, ses origines, ses motivations, ses expériences passées...">{{ old('biography', $character->biography) }}</textarea>
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 8px;">
                            ...
                            <div id="bioCounter" style="font-size: 0.8rem; color: var(--dune-sand-dark);">
                                <span id="bioLength">{{ strlen(old('biography', $character->biography ?? '')) }}</span>/5000 caractères
                            </div>
                        </div>
                        @error('biography')
                            <div class="field-error" style="color: #f44336; font-size: 0.8rem; margin-top: 5px;">{{ $message }}</div>
                ...
                <div class="form-section dune-panel" style="padding: 30px;">
                        ...
                        @foreach(\Azuriom\Plugin\DuneRp\Models\Character::SPECIAL_ABILITIES as $abilityKey => $abilityName)
                            <div class="ability-option">
                                <input type="checkbox" id="ability_{{ $abilityKey }}" name="special_abilities[]" value="{{ $abilityKey }}" 
                                       {{ in_array($abilityKey, old('special_abilities', $character->special_abilities ?? [])) ? 'checked' : '' }} style="display: none;">
                                <label for="ability_{{ $abilityKey }}" class="ability-card" style="display: block; padding: 20px; border: 2px solid rgba(230,126,34,0.3); border-radius: 10px; cursor: pointer; transition: all 0.3s; background: rgba(230,126,34,0.05);">
                                    <div style="display: flex; align-items: center; margin-bottom: 10px;">
                                        ...
                                        @if(in_array($abilityKey, $character->special_abilities ?? []))
                                            <span style="margin-left: auto; padding: 2px 6px; background: var(--dune-spice); color: white; border-radius: 8px; font-size: 0.6rem; font-weight: bold;">ACTUEL</span>
                                        @endif
                                    </div>
                    ...
                    <div id="abilityCounter" style="text-align: center; margin-top: 15px; color: var(--dune-sand-dark); font-size: 0.9rem;">
                        <span id="selectedAbilities">{{ count($character->special_abilities ?? []) }}</span>/5 capacités sélectionnées
                    </div>
                    ...
                    <div class="form-grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px;">
                        ...
                        <div class="form-group">
                            ...
                            <div class="avatar-upload-area" style="border: 2px dashed var(--dune-sand); border-radius: 10px; padding: 30px; text-align: center; background: rgba(214,166,95,0.05);">
                                <div id="avatarPreview" style="width: 120px; height: 120px; margin: 0 auto 15px; border: 3px solid var(--character-house-color, var(--dune-sand)); border-radius: 50%; overflow: hidden;">
                                    @if($character->avatar_url)
                                        <img src="{{ $character->getImageUrl() }}" alt="{{ $character->name }}" style="width: 100%; height: 100%; object-fit: cover;">
                                    @else
                                        <div style="width: 100%; height: 100%; background: linear-gradient(45deg, var(--dune-sand), var(--dune-spice)); display: flex; align-items: center; justify-content: center;">
                                ...
                                <label for="avatar" class="dune-button secondary" style="cursor: pointer;">
                                    <i class="bi bi-upload"></i> {{ $character->avatar_url ? 'Changer l\'avatar' : 'Ajouter un avatar' }}
                                </label>
            ...
            <div class="form-actions dune-panel" style="padding: 25px; text-align: center; margin-top: 30px;">
                <div style="display: flex; gap: 20px; justify-content: center; flex-wrap: wrap;">
                    <a href="{{ route('dune-rp.characters.my') }}" class="dune-button secondary" style="min-width: 150px;">
                ...
                <div style="margin-top: 20px; font-size: 0.9rem; color: var(--dune-sand-dark); max-width: 600px; margin-left: auto; margin-right: auto;">
                    ...
                    Les modifications importantes (changement de maison) nécessiteront une nouvelle validation.
                </div>
            </div>
        </form>
        
        {{-- Quick Actions --}}
        <div class="quick-actions" style="display: flex; gap: 15px; justify-content: center; margin-top: 20px; flex-wrap: wrap;">
            @if($character->is_approved && $character->is_public)
                <a href="{{ route('dune-rp.characters.show', $character) }}" class="dune-button secondary">
                    <i class="bi bi-eye"></i> Voir le Profil Public
                </a>
            @endif
            
            @if($character->house)
                <a href="{{ route('dune-rp.houses.show', $character->house) }}" class="dune-button secondary">
...
input:checked + .toggle-slider {
    background-color: var(--character-house-color, var(--dune-spice));
}
...
/* House change warning animation */
.house-change-warning {
...
// House change detection
const originalHouseId = '{{ $character->house_id }}';
document.querySelectorAll('input[name="house_id"]').forEach(input => {
    input.addEventListener('change', function() {
        const warning = document.querySelector('.house-change-warning');
        if (this.value !== originalHouseId) {
            if (!warning) {
                const newWarning = document.createElement('div');
                newWarning.className = 'house-change-warning';
                newWarning.style.cssText = 'margin-bottom: 20px; padding: 15px; background: rgba(255,152,0,0.2); border: 2px solid #ff9800; border-radius: 8px; animation: pulseWarning 2s infinite;';
                newWarning.innerHTML = '<i class="bi bi-exclamation-triangle" style="color: #ff9800;"></i> <strong style="color: #ff9800;">Attention:</strong> Changer de maison nécessitera une nouvelle approbation par un administrateur.';
                document.querySelector('.house-selection').insertBefore(newWarning, document.querySelector('.houses-grid'));
            }
        } else if (warning) {
            warning.remove();
        }
...
// Form validation with changes detection
let hasChanges = false;
document.querySelectorAll('input, textarea, select').forEach(input => {
    const originalValue = input.type === 'checkbox' ? input.defaultChecked : input.defaultValue;
    input.addEventListener('change', function() {
